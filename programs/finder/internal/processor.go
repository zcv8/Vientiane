...
func genItemProcessors(dirPath string)[]module.ProcessItem {
	savePicture := func(item structure.Item) (result structure.Item, err error) {
		if item == nil {
	...
	recordPicture := func(item structure.Item) (result structure.Item, err error) {
		v := item["file_path"]
		path, ok := v.(string)
		if !ok {
		...
		fmt.Printf("Saved file: %s, size: %d byte(s).", path, size)
		return nil, nil
	}
